...
using HqPocket.Mvvm.ComponentModel.__Internals;
...
public abstract class ObservableObject : INotifyPropertyChanged, INotifyPropertyChanging
{
    ...
    /// </remarks>
    protected bool SetValue<T>([NotNullIfNotNull(nameof(newValue))] ref T field, T newValue, Action? changedCallback = null, [CallerMemberName] string? propertyName = null)
    {
        // We duplicate the code here instead of calling the overload because we can't
        // guarantee that the invoked SetValue<T> will be inlined, and we need the JIT
        ...
        // This is the fastest SetValue<T> overload so we particularly care about
    ...
    /// raises the <see cref="PropertyChanging"/> event, updates the property with the new
    /// value, then raises the <see cref="PropertyChanged"/> event.
    /// See additional notes about this overload in <see cref="SetValue{T}(ref T,T,string)"/>.
    /// </summary>
    /// <typeparam name="T">The type of the property that changed.</typeparam>
    /// <param name="field">The field storing the property's value.</param>
    /// <param name="newValue">The property's value after the change occurred.</param>
    ...
    protected bool SetValue<T>([NotNullIfNotNull(nameof(newValue))] ref T field, T newValue, IEqualityComparer<T> comparer, Action? changedCallback = null, [CallerMemberName] string? propertyName = null)
    {
    ...
    /// raises the <see cref="PropertyChanging"/> event, updates the property with the new
    /// value, then raises the <see cref="PropertyChanged"/> event.
    /// This overload is much less efficient than <see cref="SetValue{T}(ref T,T,string)"/> and it
    ...
    /// the <see cref="SetValue{TModel,T}(T,T,TModel,Action{TModel,T},string?)"/> whenever possible
    ...
    protected bool SetValue<T>(T oldValue, T newValue, Action<T> callback, Action? changedCallback = null, [CallerMemberName] string? propertyName = null)
    {
    ...
    /// raises the <see cref="PropertyChanging"/> event, updates the property with the new
    /// value, then raises the <see cref="PropertyChanged"/> event.
    /// See additional notes about this overload in <see cref="SetValue{T}(T,T,Action{T},string)"/>.
    /// </summary>
    /// <typeparam name="T">The type of the property that changed.</typeparam>
    /// <param name="oldValue">The current property value.</param>
    /// <param name="newValue">The property's value after the change occurred.</param>
    ...
    protected bool SetValue<T>(T oldValue, T newValue, IEqualityComparer<T> comparer, Action<T> callback, Action? changedCallback = null, [CallerMemberName] string? propertyName = null)
    {
        ArgumentNullException.ThrowIfNull(comparer);
        ArgumentNullException.ThrowIfNull(callback);
    ...
    /// <see cref="PropertyChanged"/> event. The behavior mirrors that of <see cref="SetValue{T}(ref T,T,string)"/>,
    ...
    /// <see cref="SetValue{T}(ref T,T,string)"/>, and it should only be used when relaying properties to a model that
    ...
    protected bool SetValue<TModel, T>(T oldValue, T newValue, TModel model, Action<TModel, T> callback, Action? changedCallback = null, [CallerMemberName] string? propertyName = null)
        where TModel : class
    {
    ...
    /// <see cref="PropertyChanged"/> event. The behavior mirrors that of <see cref="SetValue{T}(ref T,T,string)"/>,
    /// with the difference being that this method is used to relay properties from a wrapped model in the
    /// current instance. See additional notes about this overload in <see cref="SetValue{TModel,T}(T,T,TModel,Action{TModel,T},string)"/>.
    /// </summary>
    /// <typeparam name="TModel">The type of model whose property (or field) to set.</typeparam>
    /// <typeparam name="T">The type of property (or field) to set.</typeparam>
    /// <param name="oldValue">The current property value.</param>
    ...
    protected bool SetValue<TModel, T>(T oldValue, T newValue, IEqualityComparer<T> comparer, TModel model, Action<TModel, T> callback, Action? changedCallback = null, [CallerMemberName] string? propertyName = null)
        where TModel : class
    {
        ArgumentNullException.ThrowIfNull(comparer);
        ArgumentNullException.ThrowIfNull(model);
    ...
    /// </remarks>
    protected bool SetValueAndNotifyOnCompletion([NotNull] ref TaskNotifier? taskNotifier, Task? newValue, [CallerMemberName] string? propertyName = null)
    {
        ...
        return SetValueAndNotifyOnCompletion(taskNotifier ??= new TaskNotifier(), newValue, null, propertyName);
    }
    ...
    /// field for a property). If the value has changed, raises the <see cref="PropertyChanging"/>
    /// event, updates the field and then raises the <see cref="PropertyChanged"/> event.
    /// This method is just like <see cref="SetValueAndNotifyOnCompletion(ref TaskNotifier,Task,string)"/>,
    ...
    /// </remarks>
    /// <exception cref="System.ArgumentNullException">Thrown if <paramref name="callback"/> is <see langword="null"/>.</exception>
    protected bool SetValueAndNotifyOnCompletion([NotNull] ref TaskNotifier? taskNotifier, Task? newValue, Action<Task?> callback, [CallerMemberName] string? propertyName = null)
    {
        ArgumentNullException.ThrowIfNull(callback);

        return SetValueAndNotifyOnCompletion(taskNotifier ??= new TaskNotifier(), newValue, callback, propertyName);
    }
    ...
    /// </remarks>
    protected bool SetValueAndNotifyOnCompletion<T>([NotNull] ref TaskNotifier<T>? taskNotifier, Task<T>? newValue, [CallerMemberName] string? propertyName = null)
    {
        return SetValueAndNotifyOnCompletion(taskNotifier ??= new TaskNotifier<T>(), newValue, null, propertyName);
    }
    ...
    /// field for a property). If the value has changed, raises the <see cref="PropertyChanging"/>
    /// event, updates the field and then raises the <see cref="PropertyChanged"/> event.
    /// This method is just like <see cref="SetValueAndNotifyOnCompletion{T}(ref TaskNotifier{T},Task{T},string)"/>,
    /// with the difference being an extra <see cref="Action{T}"/> parameter with a callback being invoked
    /// either immediately, if the new task has already completed or is <see langword="null"/>, or upon completion.
    /// </summary>
    /// <typeparam name="T">The type of result for the <see cref="Task{TResult}"/> to set and monitor.</typeparam>
    ...
    /// </remarks>
    /// <exception cref="System.ArgumentNullException">Thrown if <paramref name="callback"/> is <see langword="null"/>.</exception>
    protected bool SetValueAndNotifyOnCompletion<T>([NotNull] ref TaskNotifier<T>? taskNotifier, Task<T>? newValue, Action<Task<T>?> callback, [CallerMemberName] string? propertyName = null)
    {
        ArgumentNullException.ThrowIfNull(callback);

        return SetValueAndNotifyOnCompletion(taskNotifier ??= new TaskNotifier<T>(), newValue, callback, propertyName);
    }
