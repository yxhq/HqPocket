...
namespace HqPocket.Mvvm.ComponentModel;
...
public abstract class ObservableValidator : ObservableObject, INotifyDataErrorInfo
{
    ...
    /// <remarks>
    /// This method is just like <see cref="ObservableObject.SetValue{T}(ref T,T,string)"/>, just with the addition
    ...
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool SetValue<T>([NotNullIfNotNull(nameof(newValue))] ref T field, T newValue, bool validate, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
    {
        ArgumentNullException.ThrowIfNull(propertyName);

        bool propertyChanged = SetValue(ref field, newValue, changedCallback, propertyName);
    ...
    /// raises the <see cref="ObservableObject.PropertyChanging"/> event, updates the property with
    /// the new value, then raises the <see cref="ObservableObject.PropertyChanged"/> event.
    /// See additional notes about this overload in <see cref="SetValue{T}(ref T,T,bool,string)"/>.
    /// </summary>
    /// <typeparam name="T">The type of the property that changed.</typeparam>
    /// <param name="field">The field storing the property's value.</param>
    /// <param name="newValue">The property's value after the change occurred.</param>
    ...
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool SetValue<T>([NotNullIfNotNull(nameof(newValue))] ref T field, T newValue, IEqualityComparer<T> comparer, bool validate, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
    {
        ArgumentNullException.ThrowIfNull(comparer);
        ArgumentNullException.ThrowIfNull(propertyName);

        bool propertyChanged = SetValue(ref field, newValue, comparer, changedCallback, propertyName);
    ...
    /// the <see cref="ObservableObject.SetValue{T}(T,T,Action{T},string)"/> method, this overload should only be
    /// used when <see cref="ObservableObject.SetValue{T}(ref T,T,string)"/> can't be used directly.
    /// </summary>
    /// <typeparam name="T">The type of the property that changed.</typeparam>
    ...
    /// <returns><see langword="true"/> if the property was changed, <see langword="false"/> otherwise.</returns>
    /// <remarks>
    /// This method is just like <see cref="ObservableObject.SetValue{T}(T,T,Action{T},string)"/>, just with the addition
    ...
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool SetValue<T>(T oldValue, T newValue, Action<T> callback, bool validate, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
    {
        ArgumentNullException.ThrowIfNull(callback);
        ArgumentNullException.ThrowIfNull(propertyName);

        bool propertyChanged = SetValue(oldValue, newValue, callback, changedCallback, propertyName);
    ...
    /// raises the <see cref="ObservableObject.PropertyChanging"/> event, updates the property with
    /// the new value, then raises the <see cref="ObservableObject.PropertyChanged"/> event.
    /// See additional notes about this overload in <see cref="SetValue{T}(T,T,Action{T},bool,string)"/>.
    /// </summary>
    /// <typeparam name="T">The type of the property that changed.</typeparam>
    /// <param name="oldValue">The current property value.</param>
    /// <param name="newValue">The property's value after the change occurred.</param>
    ...
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool SetValue<T>(T oldValue, T newValue, IEqualityComparer<T> comparer, Action<T> callback, bool validate, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
    {
        ArgumentNullException.ThrowIfNull(comparer);
        ArgumentNullException.ThrowIfNull(callback);
        ArgumentNullException.ThrowIfNull(propertyName);

        bool propertyChanged = SetValue(oldValue, newValue, comparer, callback, changedCallback, propertyName);
    ...
    /// <see cref="ObservableObject.SetValue{TModel,T}(T,T,TModel,Action{TModel,T},string)"/>, with the difference being that this
    /// method is used to relay properties from a wrapped model in the current instance. For more info, see the docs for
    /// <see cref="ObservableObject.SetValue{TModel,T}(T,T,TModel,Action{TModel,T},string)"/>.
    /// </summary>
    ...
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool SetValue<TModel, T>(T oldValue, T newValue, TModel model, Action<TModel, T> callback, bool validate, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
        where TModel : class
    {
        ...
        bool propertyChanged = SetValue(oldValue, newValue, model, callback, changedCallback, propertyName);
    ...
    /// raises the <see cref="ObservableObject.PropertyChanging"/> event, updates the property and then raises the
    /// <see cref="ObservableObject.PropertyChanged"/> event. The behavior mirrors that of
    /// <see cref="ObservableObject.SetValue{TModel,T}(T,T,IEqualityComparer{T},TModel,Action{TModel,T},string)"/>,
    ...
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool SetValue<TModel, T>(T oldValue, T newValue, IEqualityComparer<T> comparer, TModel model, Action<TModel, T> callback, bool validate, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
        where TModel : class
    {
        ArgumentNullException.ThrowIfNull(comparer);
        ArgumentNullException.ThrowIfNull(model);
        ...
        bool propertyChanged = SetValue(oldValue, newValue, comparer, model, callback, changedCallback, propertyName);
    ...
    /// <see cref="ObservableObject.SetValue{T}(ref T,T,string?)"/> is called, otherwise no state change is performed.
    /// </summary>
    /// <typeparam name="T">The type of the property that changed.</typeparam>
    /// <param name="field">The field storing the property's value.</param>
    /// <param name="newValue">The property's value after the change occurred.</param>
    ...
    /// <exception cref="System.ArgumentNullException">Thrown if <paramref name="propertyName"/> is <see langword="null"/>.</exception>
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool TrySetValue<T>(ref T field, T newValue, out IReadOnlyCollection<ValidationResult> errors, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
    {
        ArgumentNullException.ThrowIfNull(propertyName);

        return TryValidateProperty(newValue, propertyName, out errors) &&
               SetValue(ref field, newValue, changedCallback, propertyName);
    }

    /// <summary>
    /// Tries to validate a new value for a specified property. If the validation is successful,
    /// <see cref="ObservableObject.SetValue{T}(ref T,T,IEqualityComparer{T},string?)"/> is called, otherwise no state change is performed.
    /// </summary>
    /// <typeparam name="T">The type of the property that changed.</typeparam>
    /// <param name="field">The field storing the property's value.</param>
    /// <param name="newValue">The property's value after the change occurred.</param>
    ...
    /// <exception cref="System.ArgumentNullException">Thrown if <paramref name="comparer"/> or <paramref name="propertyName"/> are <see langword="null"/>.</exception>
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool TrySetValue<T>(ref T field, T newValue, IEqualityComparer<T> comparer, out IReadOnlyCollection<ValidationResult> errors, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
    {
        ArgumentNullException.ThrowIfNull(comparer);
        ArgumentNullException.ThrowIfNull(propertyName);

        return TryValidateProperty(newValue, propertyName, out errors) &&
               SetValue(ref field, newValue, comparer, changedCallback, propertyName);
    }

    /// <summary>
    /// Tries to validate a new value for a specified property. If the validation is successful,
    /// <see cref="ObservableObject.SetValue{T}(T,T,Action{T},string?)"/> is called, otherwise no state change is performed.
    /// </summary>
    /// <typeparam name="T">The type of the property that changed.</typeparam>
    /// <param name="oldValue">The current property value.</param>
    /// <param name="newValue">The property's value after the change occurred.</param>
    ...
    /// <exception cref="System.ArgumentNullException">Thrown if <paramref name="callback"/> or <paramref name="propertyName"/> are <see langword="null"/>.</exception>
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool TrySetValue<T>(T oldValue, T newValue, Action<T> callback, out IReadOnlyCollection<ValidationResult> errors, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
    {
        ArgumentNullException.ThrowIfNull(callback);
        ArgumentNullException.ThrowIfNull(propertyName);

        return TryValidateProperty(newValue, propertyName, out errors) &&
               SetValue(oldValue, newValue, callback, changedCallback, propertyName);
    }

    /// <summary>
    /// Tries to validate a new value for a specified property. If the validation is successful,
    /// <see cref="ObservableObject.SetValue{T}(T,T,IEqualityComparer{T},Action{T},string?)"/> is called, otherwise no state change is performed.
    /// </summary>
    /// <typeparam name="T">The type of the property that changed.</typeparam>
    /// <param name="oldValue">The current property value.</param>
    /// <param name="newValue">The property's value after the change occurred.</param>
    ...
    /// <exception cref="System.ArgumentNullException">Thrown if <paramref name="comparer"/>, <paramref name="callback"/> or <paramref name="propertyName"/> are <see langword="null"/>.</exception>
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool TrySetValue<T>(T oldValue, T newValue, IEqualityComparer<T> comparer, Action<T> callback, out IReadOnlyCollection<ValidationResult> errors, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
    {
        ArgumentNullException.ThrowIfNull(comparer);
        ArgumentNullException.ThrowIfNull(callback);
        ArgumentNullException.ThrowIfNull(propertyName);
        ...
        return TryValidateProperty(newValue, propertyName, out errors) &&
               SetValue(oldValue, newValue, comparer, callback, changedCallback, propertyName);
    }

    /// <summary>
    /// Tries to validate a new value for a specified property. If the validation is successful,
    /// <see cref="ObservableObject.SetValue{TModel,T}(T,T,TModel,Action{TModel,T},string?)"/> is called, otherwise no state change is performed.
    /// </summary>
    /// <typeparam name="TModel">The type of model whose property (or field) to set.</typeparam>
    /// <typeparam name="T">The type of property (or field) to set.</typeparam>
    /// <param name="oldValue">The current property value.</param>
    ...
    /// <exception cref="System.ArgumentNullException">Thrown if <paramref name="model"/>, <paramref name="callback"/> or <paramref name="propertyName"/> are <see langword="null"/>.</exception>
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool TrySetValue<TModel, T>(T oldValue, T newValue, TModel model, Action<TModel, T> callback, out IReadOnlyCollection<ValidationResult> errors, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
        where TModel : class
    {
        ArgumentNullException.ThrowIfNull(model);
        ArgumentNullException.ThrowIfNull(callback);
        ...
        return TryValidateProperty(newValue, propertyName, out errors) &&
               SetValue(oldValue, newValue, model, callback, changedCallback, propertyName);
    }

    /// <summary>
    /// Tries to validate a new value for a specified property. If the validation is successful,
    /// <see cref="ObservableObject.SetValue{TModel,T}(T,T,IEqualityComparer{T},TModel,Action{TModel,T},string?)"/> is called, otherwise no state change is performed.
    /// </summary>
    /// <typeparam name="TModel">The type of model whose property (or field) to set.</typeparam>
    /// <typeparam name="T">The type of property (or field) to set.</typeparam>
    /// <param name="oldValue">The current property value.</param>
    ...
    /// <exception cref="System.ArgumentNullException">Thrown if <paramref name="comparer"/>, <paramref name="model"/>, <paramref name="callback"/> or <paramref name="propertyName"/> are <see langword="null"/>.</exception>
    [RequiresUnreferencedCode("The type of the current instance cannot be statically discovered.")]
    protected bool TrySetValue<TModel, T>(T oldValue, T newValue, IEqualityComparer<T> comparer, TModel model, Action<TModel, T> callback, out IReadOnlyCollection<ValidationResult> errors, Action? changedCallback = null, [CallerMemberName] string propertyName = null!)
        where TModel : class
    {
        ArgumentNullException.ThrowIfNull(comparer);
        ArgumentNullException.ThrowIfNull(model);
        ...
        return TryValidateProperty(newValue, propertyName, out errors) &&
               SetValue(oldValue, newValue, comparer, model, callback, changedCallback, propertyName);
    }
    ...
    [RequiresUnreferencedCode(
        "This method requires the generated HqPocket.Mvvm.ComponentModel.__Internals.__ObservableValidatorExtensions type not to be removed to use the fast path. " +
    ...
    protected void ValidateAllProperties()
    {
        ...
        {
            if (type.Assembly.GetType("HqPocket.Mvvm.ComponentModel.__Internals.__ObservableValidatorExtensions") is Type extensionsType &&
