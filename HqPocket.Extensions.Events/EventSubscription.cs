...
public class EventSubscription : IEventSubscription
{
    ...
    public EventSubscription(IDelegateReference actionReference)
    {
        ...
        if (actionReference.Target is not System.Action)
            throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, InternalResources.InvalidDelegateRerefenceTypeException, typeof(Action).FullName), nameof(actionReference));
...
public class EventSubscription<TPayload> : IEventSubscription
{
    ...
    public EventSubscription(IDelegateReference actionReference, IDelegateReference filterReference)
    {
        ...
        if (actionReference.Target is not Action<TPayload?>)
            throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, InternalResources.InvalidDelegateRerefenceTypeException, typeof(Action<TPayload>).FullName), nameof(actionReference));

        ArgumentNullException.ThrowIfNull(filterReference);
        if (filterReference.Target is not Predicate<TPayload?>)
            throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, InternalResources.InvalidDelegateRerefenceTypeException, typeof(Predicate<TPayload>).FullName), nameof(filterReference));
